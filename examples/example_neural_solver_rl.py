...
def get_args():
    ...
    parser.add_argument(
        "--rl-cfg", 
        default="./rl_cfg/Cartpole/cartpole.yaml", 
        type=str, 
...
def load_rl_config(args):
    ...
    if args.exp_name is not None:
        ...
        rl_cfg['rl']['config']['full_experiment_name'] = f"{rl_cfg['env']['env_name']}PPO/{get_time_stamp()}"
...
"""
Load neural_solver_cfg and set neural model.
Construct the neural env.
"""
def construct_env(env_specs, device, args):
    # copy seed into newton_env_cfg
    if "newton_env_cfg" not in env_specs:
        env_specs["newton_env_cfg"] = {}
    env_specs["newton_env_cfg"]["seed"] = args.seed
    env_specs["newton_env_cfg"]["setup_viewer"] = False
    ...
    env = NeuralEnvironment(
        ...
        newton_env_cfg = env_specs["newton_env_cfg"],

...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Visualize real-data and NeRD cube-tossing trajectories together in "
            "the Warp viewer using two differently colored cubes. NeRD rolls out "
...
def build_real2nerd_trajectories(args: argparse.Namespace) -> Real2NeRDTrajectories:
    reference = normalize_trajectory_states(
        load_trajectory(
            Path(args.real_hdf5)
            if Path(args.real_hdf5).is_absolute()
            else REPO_ROOT / args.real_hdf5,
...
def main() -> None:
    ...
    trajectories = build_real2nerd_trajectories(args)

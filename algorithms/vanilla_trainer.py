...
from utils.attention_visualization import save_attention_maps_image

def get_neural_solver_cfg(cfg):
    if 'neural_solver_cfg' in cfg['env']:
        return cfg['env']['neural_solver_cfg']
    if 'neural_integrator_cfg' in cfg['env']:
        return cfg['env']['neural_integrator_cfg']
    raise KeyError(
...
class VanillaTrainer:
    def __init__(
        ...
        self.neural_solver = neural_env.solver_neural
        self.neural_integrator = self.neural_solver
        self.neural_solver_cfg = get_neural_solver_cfg(cfg)

        # check if gravity_dir_body is included in input if using body frame
        if self.neural_solver_cfg.get('states_frame') == 'body':
        ...
        if model_checkpoint_path is None:
            self.neural_model = self.build_neural_model_from_cfg(cfg)
        else:
            ...
            checkpoint_model, robot_name, self.resume_state = \
                self.unpack_checkpoint(checkpoint)
            if robot_name != self.neural_env.robot_name:
                raise ValueError(
                    "Loaded checkpoint robot_name={} does not match env robot_name={}."
                    .format(robot_name, self.neural_env.robot_name)
                )
            if self.resume_training:
                self.neural_model = checkpoint_model
                self.neural_model.to(self.device)
            else:
                self.neural_model = self.build_neural_model_from_cfg(cfg)
                self.neural_model.load_state_dict(checkpoint_model.state_dict())
                self.neural_model.to(self.device)
                if getattr(checkpoint_model, "input_rms", None) is not None:
                    self.neural_model.input_rms = checkpoint_model.input_rms
                if getattr(checkpoint_model, "output_rms", None) is not None:
                    self.neural_model.output_rms = checkpoint_model.output_rms

        print('Model = \n', self.neural_model)
        print('# Model Parameters = ', num_params_torch_model(self.neural_model))

        self.neural_solver.set_neural_model(self.neural_model)
        ...
        if cli_cfg['train']:
            ...
            if (
                ...
                and not cli_cfg.get("skip_check_log_override", False)
            ):
            ...
            self.attention_log_dir = os.path.join(self.log_dir, 'attention_maps')
            os.makedirs(self.attention_log_dir, exist_ok = True)
            
            self.summary_log_dir = os.path.join(self.log_dir, 'summaries')
            os.makedirs(self.summary_log_dir, exist_ok = True)

            # save config
            cfg_path = os.path.join(self.log_dir, 'cfg.yaml')
            if not self.resume_training or not os.path.exists(cfg_path):
            ...
            self.attention_map_interval = max(1, self.log_interval)
            ...
            self.restore_training_state_if_available()
            ...
            for valid_dataset_name in self.valid_datasets.keys():
                ...
                if not self.resume_training or not os.path.exists(fp_path):
                    fp = open(fp_path, 'w')
                    fp.close()
    ...
    def build_neural_model_from_cfg(self, cfg: dict):
    ...
    def infer_epoch_from_legacy_checkpoint(self):
        if self.model_checkpoint_path is None:
            return -1

        checkpoint_name = os.path.splitext(
            os.path.basename(self.model_checkpoint_path)
        )[0]

        if checkpoint_name.startswith('model_epoch'):
            try:
                return int(checkpoint_name[len('model_epoch'):])
            except ValueError:
                return -1
        ...
        if checkpoint_name == 'best_eval_model':
            epoch_log_path = os.path.join(
                self.model_log_dir,
                'saved_best_eval_model_epochs.txt',
            )
        elif (
            checkpoint_name.startswith('best_valid_')
            and checkpoint_name.endswith('_model')
        ):
            valid_dataset_name = checkpoint_name[len('best_valid_'):-len('_model')]
            epoch_log_path = os.path.join(
                self.model_log_dir,
                f'saved_best_valid_{valid_dataset_name}_model_epochs.txt',
            )
        ...
        if not os.path.isfile(epoch_log_path):
            return -1

        with open(epoch_log_path, 'r') as file_obj:
            epochs = [line.strip() for line in file_obj.readlines() if line.strip()]
    ...
    def restore_training_state_if_available(self):
        if not self.resume_training:
            return

        if self.resume_state is None:
            inferred_epoch = self.infer_epoch_from_legacy_checkpoint()
            if inferred_epoch >= 0:
                self.start_epoch = inferred_epoch + 1
            print_warning(
                "Resume requested, but checkpoint has no trainer_state. "
        ...
        optimizer_state = self.resume_state.get('optimizer_state_dict')
        if optimizer_state is not None:
            self.optimizer.load_state_dict(optimizer_state)

        checkpoint_epoch = int(self.resume_state.get('epoch', -1))
        self.start_epoch = max(0, checkpoint_epoch + 1)

        best_valid_losses = self.resume_state.get('best_valid_losses', {})
        for valid_dataset_name in self.valid_datasets.keys():
            if valid_dataset_name in best_valid_losses:
                self.best_valid_losses[valid_dataset_name] = best_valid_losses[valid_dataset_name]

        if 'best_eval_error' in self.resume_state:
            self.best_eval_error = self.resume_state['best_eval_error']

        self.restore_rng_state(self.resume_state.get('rng_state'))
        print_info(
            "Resumed training state from epoch {}.".format(checkpoint_epoch)
        )
    ...
    def compute_dataset_statistics(self, dataset):
        ...
        stats_batch_size = min(len(dataset), max(1, self.batch_size))
    ...
    def preprocess_data_batch(self, data):
        ...
        data = self.neural_solver.process_neural_model_inputs(data)

        # calculate prediction target from neural env
        data['target'] = self.neural_solver.convert_next_states_to_prediction(
                            states = data['states'], 
                        ...
                        with torch.no_grad():
                            ...
                            if self.truncate_grad:
                                ...
                                grad_norm_after_clip = grad_norm(
                                    self.neural_solver.neural_model.parameters()
                                ) 
    ...
    @torch.no_grad()
    def save_attention_map_snapshot(self, epoch, dataloader, dataset_name):
        if not getattr(self.neural_model, "is_transformer", False):
            return
        ...
        model_was_training = self.neural_model.training
        self.neural_model.eval()
        self.neural_model.set_attention_capture(True)
        try:
            data = self.preprocess_data_batch(raw_batch)
            self.neural_model(data)
            attention_maps = self.neural_model.get_attention_maps()
        finally:
            self.neural_model.set_attention_capture(False)
            if model_was_training:
                self.neural_model.train()

        output_path = os.path.join(
            self.attention_log_dir,
            f"epoch_{epoch:04d}_{dataset_name}_attention.png"
        )
        saved_path = save_attention_maps_image(
            attention_maps=attention_maps,
            output_path=output_path,
            title=f"Attention Maps - Epoch {epoch}",
            source_name=f"dataset={dataset_name}",
        )
        if saved_path is not None:
            print_info(f"Saved attention map: {saved_path}")
    ...
    def train(self):
        ...
        for valid_dataset_name in self.valid_datasets.keys():
            valid_loaders[valid_dataset_name] = DataLoader(
                ...
                shuffle = True,
                num_workers = self.num_data_workers,
                drop_last = len(self.valid_datasets[valid_dataset_name]) >= self.batch_size
            )
        ...
        for epoch in range(self.start_epoch, self.num_epochs):
            ...
            with TimeProfiler(self.time_report, 'epoch'):
                ...
                if (
                    getattr(self.neural_model, "is_transformer", False)
                    and epoch % self.attention_map_interval == 0
                ):
                    if len(self.valid_datasets) > 0:
                        attention_dataset_name = next(iter(self.valid_datasets.keys()))
                        self.save_attention_map_snapshot(
                            epoch,
                            valid_loaders[attention_dataset_name],
                            attention_dataset_name,
                        )
                    else:
                        self.save_attention_map_snapshot(
                            epoch,
        ...
        self.save_training_checkpoint(last_completed_epoch)
    ...
    def test(self):
        ...
        eval_error, _, _ = self.evaluator.evaluate_action_mode(
    ...
    def save_training_checkpoint(self, epoch: int, filename: str = "latest_checkpoint.pt"):
        trainer_state = {
            'epoch': int(epoch),
            'optimizer_state_dict': self.optimizer.state_dict(),
            'best_valid_losses': dict(self.best_valid_losses),
            'best_eval_error': float(self.best_eval_error),
        ...
        torch.save(checkpoint, os.path.join(self.model_log_dir, filename))
